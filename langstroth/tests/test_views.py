...
class TestViews(TestCase):
    ...
    @patch('langstroth.views.cache')
    def test_cache_miss(
        self, mock_cache, mock_get_status, mock_get_availability
    ):
        ...
        mock_get_status.side_effect = [self.status_f5, self.status_tempest]
        mock_cache.get.return_value = None  # All miss
        request = self.rf.get(
            "/", {'start': '2024-05-17', 'end': '2024-06-17'}
        )
    ...
    def test_cache_miss_and_fail(
        self, mock_log, mock_cache, mock_get_status, mock_get_availability
    ):
        """Check that cache and nagios calls are made on cache miss
    ...
    @patch('langstroth.views.render')
    def test_context(
        self, mock_render, mock_cache, mock_get_status, mock_get_availability
    ):
        ...
        self.assertEqual(
            {'name': 'Average', 'ok': 99.727, 'critical': 0.273},
    ...
    def test_api_down(
        ...
        self.status_f5['hosts']['accounts.rc.nectar.org.au']['services'][0][
            'status'
        ] = 'Critical'
    ...
    @patch('langstroth.views.cache')
    @patch('langstroth.views.render')
    def test_tempest_down(
        self, mock_render, mock_cache, mock_get_status, mock_get_availability
    ):
        "Check that we are detecting tempest host down"

        self.status_tempest['hosts']['tempest.rc.nectar.org.au']['services'][
            0
        ]['status'] = 'Critical'
